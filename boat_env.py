...
class BuoyantBoat(gym.Env):
    def __init__(self, kp=0.0, ki=0.0, kd=0.0, control_technique="DQN"):
        ...
        self.state = [
            np.array([0, 0, 2], dtype=np.float64),  # boat position
    ...
    def reset(self, **kwargs):
        ...
        self.state = [
            np.array([0, 0, -2], dtype=np.float64),  # boat position
            np.array([0, 0, 0], dtype=np.float64),  # boat velocity
            np.array([0, 0], dtype=np.float64),  # boat orientation
            np.array([0], dtype=np.float64),  # boat angular velocity
            np.array([0], dtype=np.float64),  # load position
        ]
        self.prev_state = [
            np.array([0, 0, -2], dtype=np.float64),  # boat position
            np.array([0, 0, 0], dtype=np.float64),  # boat velocity
            np.array([0, 0], dtype=np.float64),  # boat orientation
            np.array([0, 0], dtype=np.float64),  # boat angular velocity
            np.array([0], dtype=np.float64),  # load position
        ]
    ...
    def get_buoyancy(self):
        ...
        for i in range(2):
            for j in range(2):
                ...
                _hull_to_surface_distance = (self.wave_state[i][j] - (force_applied_y[i][j][0] - self._force_applied_coords[i][j][2]))
                ...
                print(f"Distance to water = {_hull_to_surface_distance}")
    ...
    def step(self, action):
        ...
        return copy.deepcopy(self.state), _current_buyoancy_forces, done, False, info  # TODO: self.state should be self.obs
